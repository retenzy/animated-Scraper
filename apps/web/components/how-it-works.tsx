...
type HowItWorks = NonNullable<LandingPage['howItWorks']>
...
export default function HowItWorks({ howItWorks }: { howItWorks?: HowItWorks }) {
  const heading = howItWorks?.heading || 'Simple workflow'
  const subheading = howItWorks?.subheading || 'Get started in minutes. No setup headaches or complex configurations.'
  const steps = howItWorks?.steps?.filter(Boolean).length ? howItWorks.steps : fallbackSteps
  const benefitsTitle = howItWorks?.benefitsTitle || 'Why teams love Retenzy'
  const benefits = howItWorks?.benefits?.length ? howItWorks.benefits : fallbackBenefits
    ...
    <section id="how-it-works" className="py-20 px-6 bg-card">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {steps.map((step, index) => (
            <div key={index} className="relative p-8 rounded-lg bg-background border border-border">
              <div className="absolute -top-4 -left-4 w-10 h-10 bg-foreground rounded-full flex items-center justify-center text-background text-sm font-bold">
                {step.number}
              </div>
        ...
        <div className="mt-16 p-8 rounded-lg bg-background border border-border">
          <h3 className="text-xl font-semibold text-foreground mb-6">{benefitsTitle}</h3>
